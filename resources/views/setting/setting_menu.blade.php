...
<ul class="nav nav-tabs mb-5 pb-1 overflow-auto flex-nowrap text-nowrap" id="subAnalytics" role="tablist">
    ...
    <li class="nav-item position-relative me-7 mb-3" role="presentation">
        <a class="nav-link p-0 {{ (isset($sectionName) && $sectionName == 'advanced_setting') ? 'active' : ''}}"
           href="{{ route('setting.index',['section' => 'advanced_setting']) }}">{{ __('messages.setting.advanced_setting') }}</a>
    </li>
</ul>
</div>
